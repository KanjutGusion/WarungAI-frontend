    ...
    <main class="flex-1 px-4 py-6 sm:px-6 lg:px-10">
      <div class="w-full space-y-8">
       <!-- Header -->
        ...
        <div
          v-if="error"
          class="rounded-lg border border-red-500/40 bg-red-500/10 text-red-200
        ...
        <section class="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
          ...
          <div v-if="isLoading" class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div
              v-for="n in 4"
              :key="n"
              class="p-4 bg-slate-900/40 rounded-lg border border-slate-700 animate-pulse"
            >
              <div class="h-3 w-16 bg-slate-700 rounded mb-2"></div>
              <div class="h-5 w-24 bg-slate-600 rounded"></div>
            </div>
          </div>
          ...
          <div v-else class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="p-4 bg-slate-900/40 rounded-lg border border-slate-700">
              <p class="text-xs text-slate-400">Total Omzet</p>
        ...
        <section class="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
          <div class="flex items-center justify-between gap-2">
            <h2 class="text-lg font-semibold text-white">Grafik Omzet</h2>
            <p class="text-xs text-slate-400">7 hari terakhir</p>
          </div>

          <div class="bg-slate-900/60 border border-slate-700/80 rounded-lg p-4">
